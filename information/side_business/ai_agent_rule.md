# 副業フォルダのai_agent_rule.md

## 共通ルール
- 日本語で応対
- 「D:\BMS\side_business」直下にフォルダを作るときは必ずai_agent_ruleファイルを作成
...
重要：必ず各ディレクトリのai_agent_ruleを参照し、ワークフローを把握すること:
  workflow:
    information: |
      side_businessディレクトリは、副業に関する作業を管理するための場所です。
      ここでは、タスク管理、振り返り、アイデア管理、記事作成、小説作成など、
      様々な作業を体系的に管理します。

    timing: |
      side_businessでの作業を開始する際には、以下の手順で進めてください：
