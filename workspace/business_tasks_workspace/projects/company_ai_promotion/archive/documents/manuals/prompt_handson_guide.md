...
**1.2.1.2 ケーススタディ：議事録生成**
```markdown
## 要望
- あなたは議事録作成の専門家です
- 文字起こし情報や音声データから会議内容を構造化し、正式な議事録を作成
...
- 構成要素:
  ...
  2. 議題ごとの議論概要
  3. 決定事項（★マークで強調）
  4. アクションアイテム（担当者と期限明記）
  5. 未解決課題（次回議題候補）
- 文体: ですます調（ただし決定事項は「〜とする」の断定形）
- 文字数: 500-800字
...
**1.2.4 出力結果（Output）**
- **品質評価基準**：
  - 実際にプロンプトを業務に適応する際は、以下のような評価基準を設定し、対応していく必要があります。
...
| タスク段階 | 推奨モデル | 活用ポイント |
|------------|------------|--------------|
| タスク分析、分割 | Claude 3.5 Sonnet | 自然言語理解 |
| データ処理 | Gemini flash 2.0 | 構造化処理 |
| 品質検証 | Claude 3.5 Sonnet、Claude 3.5 Haiku | 精密チェック |
...
4. **セキュリティチェックの自動化**：
   - **機密情報検出**の自動化
     - 個人情報や企業機密の漏洩を防止
   - **データ漏洩防止**のための自動フィルタリング
     - 不適切な情報の出力をブロック
   - **コンプライアンスチェック**の自動実施
     - 法規制や社内規定に準拠しているか自動検証
