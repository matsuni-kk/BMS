...
## 共通ルール
- 日本語で応対
- 「D:\BMS\workspace」直下にフォルダを作るときは必ずai_agent_ruleファイルを「D:\BMS\ai_agent_rule_format.md」をフォーマットにして作成
...
重要：必ず各ディレクトリのai_agent_ruleを参照し、ワークフローを把握すること:
  ...
  business_tasks_workspace:
    information: |
      ビジネスタスクを管理するためのディレクトリです。
      日々の業務、プロジェクト、タスクを体系的に管理します。
    timing: |
      1. タスクの整理（毎日）：
         - その日のタスクリストを確認
         - 各タスクの優先度を設定
      ...
      2. プロジェクト管理（週次）：
         - 各プロジェクトの進捗状況を確認
         - プロジェクトの課題を洗い出し
         - プロジェクト計画を見直し、必要に応じて修正
         - リソースの配分状況を確認
         - プロジェクトの遅延リスクを評価
      3. タスクの完了（随時）：
         - 完了したタスクを報告
         - タスクの結果を記録
         - タスクの実行プロセスを振り返り、改善点を特定
         - 完了したタスクをアーカイブ
    directory: workspace/business_tasks_workspace/
  
  private_tasks_workspace:
    information: |
      プライベートタスクを管理するためのディレクトリです。
      個人的なタスク、目標、プロジェクトを体系的に管理します。
    timing: |
      1. タスクの整理（毎日）：
         - その日のタスクリストを確認
         - 各タスクの優先度を設定
      ...
      3. タスクの完了（随時）：
         ...
         - タスクの実行プロセスを振り返り、改善点を特定
         - 完了したタスクをアーカイブ
    directory: workspace/private_tasks_workspace/
